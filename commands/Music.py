...
class Music(app_commands.Group):
    ...
    async def skip(self, interaction: discord.Interaction):
        ...
        try:
            ...
            voice_client.stop()
            await interaction.followup.send("Pulando a música...")
    ...
    async def play_queue(self, voice_channel: discord.VoiceChannel, voice_client: discord.VoiceClient, queue_size: int):
        logger.info("play_queue method was called: %s, %s, %s",
                    voice_channel, voice_client, queue_size)
        while queue_size > 0:
        ...
        if YTDLSource.is_url(music):
            ...
            try:
                ...
                button_list = ButtonList(
                    results, interaction, self.process_url)
    ...
    async def remove(self, interaction: discord.Interaction, title: str):
        ...
        interaction.followup.send(
            "Você não enviou essa música para a queue, logo você não tem permissão para removê-la. Pense na diversão do amiguinho também. 😐")
        return
